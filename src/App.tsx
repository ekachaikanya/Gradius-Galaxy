...
import { PilotProfile, GameView, StoryStage, StageBuff } from "./types";
...
import { Gamepad2, Trophy, Shield, Rocket, Radio, RefreshCcw } from "lucide-react";
...
export default function App() {
      ...
      if (user) {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            const defaultName = user.displayName ? user.displayName.split(" ")[0].slice(0, 16) : `Pilot-${Math.floor(100 + Math.random() * 900)}`;
            const mockProfile: PilotProfile = {
              uid: user.uid,
              commanderName: defaultName,
              highScore: 0,
              storyProgress: 0
            };
  ...
  const handleStartMission = (stage: StoryStage, buff: StageBuff | null) => {
  ...
  const handleGameFinished = async (finalScore: number, progressedChapter: number, ghostFramesStr: string) => {
    ...
    // Auto submit scores to cloud databases if logged in!
    if (currentUser) {
      ...
      try {
        ...
        if (ghostFramesStr && ghostFramesStr.length > 10) {
          const ghostPayload = {
            ...
            commanderName: commanderName.slice(0, 16),
            score: finalScore,
        ...
        // 3. Update Pilot's Profile if score exceeds their previous best
        ...
        setSaveStatus("Logs rejected. Make sure you are authenticated with proper permissions.");
      } finally {
      ...
      <AnimatePresence mode="wait">
        ...
        {activeView === GameView.MAIN_MENU && (
          ...
          >
            ...
            {/* Callsign Status Banner of active User */}
            <div className="my-6 bg-black/40 p-3 rounded-xl border border-white/5 inline-flex items-center space-x-3 text-xs font-mono">
              <span className="text-cyan-400 flex items-center space-x-1 animate-pulse"><span className="text-base">●</span> <span>PILOT SIGN-ON</span></span>
              <span className="text-zinc-400">CALLSIGN:</span>
              <strong className="text-white text-sm">{commanderName}</strong>
              {currentUser && <span className="bg-cyan-950/60 text-cyan-400 border border-cyan-800/50 rounded px-1 text-[9px]">GOOGLE COUPLING</span>}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-lg mx-auto">
              ...
              >
                ...
                <div>
                  <h3 className="text-sm font-extrabold uppercase font-display text-white group-hover:text-orange-400 transition-colors">🏆 MULTIPLAYER LOBBY</h3>
                  <p className="text-zinc-400/80 text-[11px] font-sans leading-tight mt-1">
                    Synchronous pilot conversations, regional leaderboards, and real flight ghost duels.
                  </p>
                </div>
              </button>
        ...
        {/* VIEW 3: Competitive Lobby coordination */}
